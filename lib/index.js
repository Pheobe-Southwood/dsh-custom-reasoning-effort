 ...
 * catalog with no `reasoning` capability, so the composer's official 推理等级
 * picker stays hidden even when the very same model id is reasoning-capable
 * under a builtin provider route. The catalog resolves a model's reasoning
 * capability per provider route, and the custom-provider card never writes the
 * per-model `reasoningEfforts` field the `llm-pi-ai` schema accepts, so there is
 * nothing for a custom route to inherit from.
 *
 ...
 * picker → `reasoning_effort` on the wire) does the rest with no custom UI.
 *
 ...
 * therefore the official picker — is derived from. Writing the field changes
 * the input of the shipped pipeline instead of competing with it.
 *
 * @module dsh-custom-reasoning-effort
 */
import { customModelIds, planNormalization, withoutRoutes } from './normalize.js'
  ...
  try {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        ctx.logger.info(`custom-reasoning-effort: filled reasoningEfforts on ${plan.summary.planned} model(s) across ${plan.ops.length} custom route(s)`)
        return
      } catch (error) {
 ...
 * contributes no facts, and the planner falls back to the standard set rather
 * than guessing.
 *
 * @param ctx - the plugin context.
...
async function capabilityOf(ctx, route, modelId) {
  try {
    ...
    if (reasoning === null || typeof reasoning !== 'object' || !Array.isArray(reasoning.efforts)) {
      return { reasoning: false, levels: [] }
    }
    const levels = reasoning.efforts
      .map((effort) => (effort !== null && typeof effort === 'object' ? effort.id : undefined))
      .filter((id) => typeof id === 'string' && id.length > 0)
    if (levels.length === 0) return { reasoning: false, levels: [] }
    return { reasoning: true, levels }
  } catch {
    return undefined
  }
}
